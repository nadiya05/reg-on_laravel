            ...
            @forelse ($users as $u)
                <a href="{{ route('chat.show', $u->id) }}" class="text-decoration-none text-dark">
                    <div class="d-flex align-items-center p-2 mb-2 rounded
                        ...
                        <img src="{{ $u->foto ? asset('storage/'.$u->foto) : 'https://ui-avatars.com/api/?name='.urlencode($u->name) }}"
        ...
        <div class="col-md-8 d-flex flex-column" style="height:80vh;">
            @if($activeUser)
                ...
                <div class="border-bottom p-2 d-flex align-items-center">
                    <img src="{{ $activeUser->foto ? asset('storage/'.$activeUser->foto) : 'https://ui-avatars.com/api/?name='.urlencode($activeUser->name) }}"
                         width="45" height="45" class="rounded-circle" style="object-fit:cover;">
                    <h5 class="ms-2 mb-0">{{ $activeUser->name }}</h5>

                    <form method="POST" action="{{ route('chat.clear', $activeUser->id) }}" class="ms-auto"
                          onsubmit="return confirm('Hapus semua chat user ini?')">
                        @csrf @method('DELETE')
                ...
                <div id="chat-box" class="flex-grow-1 p-3" style="overflow-y:auto;">
                    @foreach($messages as $m)
                        <div class="d-flex mb-3 {{ $m->sender=='admin' ? 'justify-content-end' : 'justify-content-start' }}">
                            ...
                            {{-- Avatar kiri (user) --}}
                            @if($m->sender=='user')
                                <img src="{{ $m->user && $m->user->foto ? asset('storage/'.$m->user->foto) : 'https://ui-avatars.com/api/?name='.urlencode($m->user->name ?? 'User') }}"
                            ...
                            <div>
                                <div class="p-2 rounded {{ $m->sender=='admin' ? 'bg-primary text-white' : 'bg-light' }}"
                                    ...
                                    {{-- tombol hapus --}}
                                    <form action="{{ route('chat.delete',$m->id) }}" method="POST"
                                          style="position:absolute; top:2px; right:2px;"
                                          onsubmit="return confirm('Hapus pesan ini?')">
                                        @csrf @method('DELETE')
                                        <button class="btn btn-sm text-danger border-0 p-0" style="font-size:10px;background:transparent;">✕</button>
                                    </form>
                                </div>
                            ...
                            {{-- Avatar kanan (admin) --}}
                            @if($m->sender=='admin')
                                <img src="{{ auth()->user()->foto ? asset('storage/'.auth()->user()->foto) : 'https://ui-avatars.com/api/?name=Admin' }}"
                ...
                {{-- SEND --}}
                <form action="{{ route('chat.send', $activeUser->id) }}" method="POST" class="d-flex border-top p-2">
                    @csrf
...
(function () {
    // subscribe ke channel milik user (activeUser)
    Echo.private('chat.{{ $activeUser->id }}')
        .listen('.message.sent', (e) => {
            ...
            const avatar = e.avatar || '{{ auth()->user()->foto ? asset("storage/".auth()->user()->foto) : "https://ui-avatars.com/api/?name=Admin" }}';
            const userAvatar = e.avatar || 'https://ui-avatars.com/api/?name=User';

            const html = `
            <div class="d-flex mb-3 ${isAdmin ? 'justify-content-end' : 'justify-content-start'}">
                ${!isAdmin ? `
                <img src="${userAvatar}" class="rounded-circle me-2" width="40" height="40" style="object-fit:cover;">
                ` : ''}
                ...
                ${isAdmin ? `
                <img src="${avatar}" class="rounded-circle ms-2" width="40" height="40" style="object-fit:cover;">
                ` : ''}
            </div>
            `;
